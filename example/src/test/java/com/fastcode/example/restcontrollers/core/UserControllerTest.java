...
import com.fastcode.example.domain.core.authorization.userpreference.IUserpreferenceManager;
...
public class UserControllerTest extends DatabaseContainerConfig{
	...
	public static void cleanup() {
		...
		em.createNativeQuery("truncate table s2.userpreference CASCADE").executeUpdate();
    ...
    public CreateUserInput createUserInput() {
		...
		userInput.setEmailAddress("pmlo@d.c");
	...
	public User createUpdateEntity() {
		User user = new User();
  		user.setEmailAddress("ppk@d.c");
		user.setFirstName("4");
		user.setId(4L);
		user.setIsActive(false);
		user.setIsEmailConfirmed(false);
	...
	public void setup() {
		...
		final UserController userController = new UserController(userAppService, userpermissionAppService, userroleAppService,
	...
	public void FindById_IdIsNotValid_ReturnStatusNotFound() {
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/user/999")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void DeleteUser_IdIsNotValid_ThrowEntityNotFoundException() {
        ...
        org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(delete("/user/999")
				.contentType(MediaType.APPLICATION_JSON))
				.andExpect(status().isOk())).hasCause(new EntityNotFoundException("There does not exist a user with a id=999"));
	...
	public void UpdateUser_UserDoesNotExist_ReturnStatusNotFound() throws Exception {
		...
		String json = ow.writeValueAsString(user);
		 ...
		 org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(
		 	...
		 	.contentType(MediaType.APPLICATION_JSON)
		 	.content(json))
			.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Unable to update. User with id=999 not found."));
	}    

	@Test
	public void UpdateUser_UserExists_ReturnStatusOk() throws Exception {
		User entity =  createUpdateEntity();
		entity.setVersiono(0L);
	...
	public void GetUserpermissions_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/user/1/userpermissions?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetUserpermissions_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(userAppService.parseUserpermissionsJoinColumn("userId")).thenReturn(joinCol);
		mvc.perform(get("/user/1/userpermissions?search=userId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetUserpermissions_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/user/1/userpermissions?search=userId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	...
	public void GetUserroles_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/user/1/userroles?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetUserroles_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(userAppService.parseUserrolesJoinColumn("userId")).thenReturn(joinCol);
		mvc.perform(get("/user/1/userroles?search=userId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetUserroles_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/user/1/userroles?search=userId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
