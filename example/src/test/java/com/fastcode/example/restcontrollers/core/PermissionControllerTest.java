...
public class PermissionControllerTest extends DatabaseContainerConfig{
	...
	public void setup() {
		...
		final PermissionController permissionController = new PermissionController(permissionAppService, rolepermissionAppService, userpermissionAppService,
	logHelper,env);
	...
	public void GetUserpermissions_searchIsNotEmptyAndPropertyIsNotValid_ThrowException() {
		...
		org.assertj.core.api.Assertions.assertThatThrownBy(() ->  mvc.perform(get("/permission/1/userpermissions?search=abc[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk())).hasCause(new Exception("Wrong URL Format: Property abc not found!"));
	...
	public void GetUserpermissions_searchIsNotEmptyAndPropertyIsValid_ReturnList() throws Exception {
        ...
        Mockito.when(permissionAppService.parseUserpermissionsJoinColumn("permissionId")).thenReturn(joinCol);
		mvc.perform(get("/permission/1/userpermissions?search=permissionId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
	    		  .andExpect(status().isOk());
	}  
	...
	public void GetUserpermissions_searchIsNotEmpty() {
	    ...
	    org.assertj.core.api.Assertions.assertThatThrownBy(() -> mvc.perform(get("/permission/1/userpermissions?search=permissionId[equals]=1&limit=10&offset=1")
				.contentType(MediaType.APPLICATION_JSON))
		.andExpect(status().isOk())).hasCause(new EntityNotFoundException("Invalid join column"));
	}    
