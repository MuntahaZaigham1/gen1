...
public class UserroleAppService implements IUserroleAppService {
	...
	public CreateUserroleOutput create(CreateUserroleInput input) {
		...
		Userrole userrole = mapper.createUserroleInputToUserrole(input);
		Role foundRole = null;
		User foundUser = null;
		...
		{			
			...
			{
				foundRole.addUserroles(userrole);
				foundUser.addUserroles(userrole);
		...
		Userrole createdUserrole = _userroleRepository.save(userrole);
		return mapper.userroleToCreateUserroleOutput(createdUserrole);
	}
	
	@Transactional(propagation = Propagation.REQUIRED)
	public UpdateUserroleOutput update(UserroleId userroleId, UpdateUserroleInput input) {

		Userrole existing = _userroleRepository.findById(userroleId).get();

		Userrole userrole = mapper.updateUserroleInputToUserrole(input);
		Role foundRole = null;
		User foundUser = null;
		...
		Userrole updatedUserrole = _userroleRepository.save(userrole);
		return mapper.userroleToUpdateUserroleOutput(updatedUserrole);
	}
	
	public boolean checkIfRoleAlreadyAssigned(User foundUser,Role foundRole)
	{
		List<Userrole> userRole = _userroleRepository.findByUserId(foundUser.getId());

		Iterator rIterator = userRole.iterator();
		while (rIterator.hasNext()) {
			Userrole ur = (Userrole) rIterator.next();
	...
	public void delete(UserroleId userroleId) {
		...
		Userrole existing = _userroleRepository.findById(userroleId).orElse(null); 
	 	
        if(existing.getRole() !=null)
        {
        existing.getRole().removeUserroles(existing);
        }
        if(existing.getUser() !=null)
        {
        existing.getUser().removeUserroles(existing);
        }
	...
	public FindUserroleByIdOutput findById(UserroleId userroleId) {
		...
		Userrole foundUserrole = _userroleRepository.findById(userroleId).orElse(null);
		if (foundUserrole == null)  
			return null; 
 	   
 	    return mapper.userroleToFindUserroleByIdOutput(foundUserrole);
	}
	...
	public GetRoleOutput getRole(UserroleId userroleId) {
		...
		Role re = foundUserrole.getRole();
		return mapper.roleToGetRoleOutput(re, foundUserrole);
	}
	...
	public GetUserOutput getUser(UserroleId userroleId) {
		...
		User re = foundUserrole.getUser();
		return mapper.userToGetUserOutput(re, foundUserrole);
	}
	
    @Transactional(propagation = Propagation.NOT_SUPPORTED)
	public List<FindUserroleByIdOutput> find(SearchCriteria search, Pageable pageable) throws Exception  {

		Page<Userrole> foundUserrole = _userroleRepository.findAll(search(search), pageable);
		List<Userrole> userroleList = foundUserrole.getContent();
		...
		Userrole userrole = userroleIterator.next();
 	    output.add(mapper.userroleToFindUserroleByIdOutput(userrole));
		}
		return output;
	}
...
//		        list.get(i).replace("%20","").trim().equals("userName") ||
	...
	protected BooleanBuilder searchKeyValuePair(QUserrole userrole, Map<String,SearchFields> map,Map<String,String> joinColumns) {
		BooleanBuilder builder = new BooleanBuilder();
        
		for (Map.Entry<String, SearchFields> details : map.entrySet()) {
			 if(details.getKey().replace("%20","").trim().equals("roleId")) {
			 	if(details.getValue().getOperator().equals("contains")) {
					builder.and(userrole.roleId.like(details.getValue().getSearchValue() + "%"));
			 ...
			 if(details.getKey().replace("%20","").trim().equals("userId")) {
			 	if(details.getValue().getOperator().equals("contains")) {
					builder.and(userrole.userId.like(details.getValue().getSearchValue() + "%"));
				} else if(details.getValue().getOperator().equals("equals") && StringUtils.isNumeric(details.getValue().getSearchValue())) {
